...
**目标**：只推进剩余业务迁移；按真实用户入口逐域建立可独立验收的 `AG-*` 原子卡，不把单个 bridge 或单个入口冒充整域生产迁移完成。当前 Apps / Workflow 与 Chat / Live Run 的真实入口 → Runtime → AgentScope 首闭环已完成，均标记为已完成且不可重派；Automation / Task Center 仅部分接入，其余业务域尚无足够证据证明已接入。
...
顺序是当前剩余工作的阶段级执行建议，不代表可以跳过每个域的 A01～A05 验收顺序。任何域在正式切换前都必须完成自己的数据、权限、幂等、失败、恢复、对账和回滚证据；本轮不实例化 HA、备份恢复、灾备、压测、容量、历史迁移或迁移运维卡片。
